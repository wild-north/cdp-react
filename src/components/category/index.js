...
const Title = ({showOpener, opened, fullIndex, title, enableEdit, remove, toggle, add, selectCategory, routeParams}) => {
  return (
    <div>
      <div className="input-holder">
        {
        ...
        <Link to={`/category/${fullIndex.split(SEPARATOR).join(LINK_SEPARATOR)}`} activeClassName="active">
          <span className="title" onClick={selectCategory(fullIndex)}>
            <small>{fullIndex}</small> {title}</span>
        </Link>
      </div>
      <div className="actions-holder">
        {
...
export default class Category extends Component {
  ...
  render() {
    const {children, index, parentIndex, item, routeParams} = this.props;
